...
GF(2^8) = Galois Field 256:
Galois Field 256 multipication in c#:
```
...
```
The function above is in the AES.OPS_AES256 class.
...
I converted 1-dimentional arrays which are Sbox and InvSbox into a 2-dimentional matrix in c#. The following code doesn't convert S-box and Inverse S-box to 2-dimentional matrix by itself, you paste the result into another document and use ctrl-h to find and replace all the following strings: ```, }``` to ```}```, and ```}{``` to ```},  {```. After I did that, I copied the result from the console to the AES.OPS_AES256 class using the same type except instead of using ```byte[]``` and ```byte[256]``` I used ```byte[,]``` and ```byte[16,16]``` so that it is a 2-dimentional matrix:
```
using System;
class Convert1DTo2D
{
    ...
    static void Main(string[] args)
    {
        ...
        for(int r=0;r<256;r++) {
            if (r%16 == 0) {
                ...
                System.Console.Write("0x" + hexValue + ", ");
            if (r%16 == 15) {
                Console.Write("}");
            }
        }
...
References: FIPS 197 AES nvlpubs.nist.gov
